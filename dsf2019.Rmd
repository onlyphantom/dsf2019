...
cleaned$DOW <- weekdays(cleaned$Order.Date)
str(cleaned)
```

```{r}
write.csv(cleaned, "processed.csv")
```

1. What is the average (mean) Sales from Corporate customers?
```{r}
corporate <- cleaned[cleaned$Segment == 'Corporate', ]
mean(corporate$Sales)
```

2. What is the average (mean) Profit from transactions on First Class shipment?
```{r}
mean(cleaned[cleaned$Ship.Mode == "First Class", "Profit"])
```

3. What percentage of our shipment to Corporate segment is on First Class?
```{r}
corporate <- cleaned[cleaned$Segment == 'Corporate',] 
round(prop.table(table(corporate$Ship.Mode)) * 100, 2)
```
Answer: 21.05%

```{r}
corporate <- cleaned[cleaned$Segment == 'Corporate',] 
nrow(corporate[corporate$Ship.Mode == "First Class", ])/nrow(corporate) * 100
```

4. How many transactions are not shipped on Same Day?
```{r}
nrow(cleaned[ cleaned$Ship.Mode != 'Same Day', ])
```

Resume at: 2.55pm

5. Among all high valued transactions (profit > 1000), which segment is most represented?
```{r}
table(cleaned[cleaned$Profit > 1000,"Segment"])
```

6. Working with dates
  - 6.1 What day of week brings us the most amount of sales in total? Answer: 21769.526
  - 6.2 What day of week brings us the most amount of sales on average?
  - 6.3 What day of week brings us the most amount of sales in volume of transactions? Answer: 17 transactions on Friday in total

7. Which Shipment Mode carries the highest profit in total / overall profit?
7.2 Which Shipment Mode on average carries the highest profit?

```{r}
output <- aggregate(Profit ~ Ship.Mode + Segment, data=cleaned, FUN=mean)
head(output, 7)
```

```{r}
output[order(output$Profit, decreasing = TRUE), ]
```
...
```{r}
plot(x=cleaned$Sales, y=cleaned$Profit)
```

```{r}
model1 <- lm(formula=Profit ~ Sales, data=cleaned)
...
```{r}
newstock <- data.frame(Sales=c(3000, 2600, 1900, 3200))
predict(model1, newstock, interval = "confidence")
```

```{r}
plot(x=cleaned$Sales, y=cleaned$Profit)
abline(coef(model1)[1], coef(model1)[2], col="dodgerblue4", lwd=2)
```
...
Build a linear model to predict horsepower (hp) using miles per gallon (mpg).

Then use predict(your_model, data.frame(mpg=20)), what is your result?


```{r}
fuelmodel <- lm(hp ~ mpg + wt, mtcars)
coef(fuelmodel)
```

hp = 349.287349 + -9.416845 * mpg + -4.168012  * wt

```{r}
predict(fuelmodel, data.frame(mpg=20, wt=3.4))
```

Two dimensions:
When you want to visualize x (numeric) and y (numeric)
  - scatterplot
When you want to visualize x (categorical) and y (numeric)
  - boxplot
  
One dimension:
To visualize distribution, use histogram
  - `hist(mtcars$mpg, breaks = 30, freq = FALSE)`
  - `boxplot(cleaned$profit)`
...
hist(mtcars$mpg, 
     breaks = 30, 
     freq = FALSE, 
     col="hotpink", main="Miles per Gallon")

abline(v=20, lwd=2, col="dodgerblue4")
```
...
ggplot(data=cleaned, aes(x=Sales, y=Profit))+
  geom_point() +
  ...
  labs(
    x='Sales (IDR)', 
    y='Estimated Profit (IDR)',
    title = 'Profit Analysis by Segment',
    subtitle = '2017-2019 Profit Analysis',
    caption = 'Business Intelligence Division, 2019'
  ) +
  theme_economist()
```
